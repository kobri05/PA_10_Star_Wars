...
> Both codes pull the title of the 4th item on the list, however using 'pluck' streamlines the code and makes it more efficient to type and use.
...
> The first code identifies a map for the vector 'starships', and then turns it into a map of doubles. The second code maps the length of the vector starships as a double initially, and the third code pipes the data and streamlines the code, creating a map of a double and piping the dataset into the command.
...
sw_ships %>% 
  ...
  #what does geom_text() do? - adds text to points (?)
  ...
  theme_minimal() +
  #what aesthetic do we want to modify the color of points? - color because it's points; fill for cols or bars, etc.
...
> The sequels have the least, and the prequels tend to use hyperdrive type vehicles more than the originals.
